...
def create_images():
    ...
    cv2.rectangle(img1, (200, 150), (440, 330), (0, 0, 255), -1)  # Green in BGR
    cv2.imwrite(os.path.join(output_dir, 'red_square_noisy.png'), img1)

    # 2. Green Circle with Noise (Matching Gazebo Green)
    img2 = np.full((480, 640, 3), 255, dtype=np.uint8)  # White background
    cv2.circle(img2, (320, 240), 75, (0, 0, 255), -1)  # Green in BGR
    # Add some noise
